...
from WebDev.tasks.aula7.api import Session, select, SQLModel, create_engine
...
def listar_tarefas_do_aluno(aluno_nusp: int):
    with Session(engine) as session:
        ...
        query = (select(Aluno).where(Aluno.aluno_nusp.contains(aluno_nusp))
        )
        ...
        # e retornando a propriedade implementada anteriormente.ip install "fastapi[standard]"